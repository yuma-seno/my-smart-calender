...
const Schedule = ({ events, selectedDate }: ScheduleProps) => {
  ...
  return (
    <Card className="p-4">
      <h3 className="text-[18px] font-bold mb-2 border-b border-gray-200 dark:border-white/10 pb-2 flex-none text-gray-600 dark:text-white">
        {isToday ? "今日の予定" : `${m}月${d}日の予定`}
      </h3>
      <div className="flex-1 overflow-y-auto space-y-2 min-h-0">
        {daysEvents.length === 0 ? (
          <div className="text-center mt-4 text-[13px] text-gray-500 dark:text-gray-300">
            予定はありません
          </div>
        ) : (
          daysEvents.map((ev: any, i: number) => (
            ...
            >
              <div
                className="text-[16px] font-mono mb-0.5"
              ...
              <div className="font-medium text-[16px] text-gray-600 dark:text-white">
                {ev.summary}
              </div>
            </div>
          ))
