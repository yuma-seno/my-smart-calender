  ...
  const buildMonthGrid = (monthDate: Date) => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        ...
        >
          <span
            className={`text-sm font-bold z-10 mb-0.5 text-center ${numClass}`}
          >
            {d}
          </span>
          <div className="w-full flex flex-col gap-x-0.5 gap-y-0.5 z-10">
            {dayEvents
              ...
              .map((ev: any, i: number) => (
                <div
                  key={i}
                  className={`
                h-4 text-[11px] leading-none px-1 flex items-center text-white truncate
            ...
            {dayEvents.length > 3 && (
              <div className="text-[9px] pl-1 text-gray-500 dark:text-gray-400">
                +{dayEvents.length - 2}
              </div>
            )}
          </div>
